...
export function createWorkflowRoutes(workflowRepository: WorkflowRepository): Router {
  const router = Router();
  ...
  router.get('/:id/status', async (req, res) => {
    ...
    try {
      const workflow = await workflowRepository.findByIdWithTasks(parsedParams.data.id);
      if (!workflow) {
